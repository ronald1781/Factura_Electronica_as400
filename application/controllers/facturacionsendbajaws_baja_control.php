...
class Facturacionsendbajaws_baja_control extends CI_Controller {
	...
	function validar($dato = null,$fecha){
	...
	function procesarbajafe($dato= NULL){
		$datap=$dato;
		$nropdc=$datap['nropdc'];
		$serie=$datap['serie'];
		$correl=$datap['correl'];
		...
		if(($stsdoct=="N")||($stsdoct=="E")){
			...
			if($filesjs!=''){
				...
				$dataenvws=['file'=>$file,'filejs'=>$filejs];
		...
		$time_end = microtime(true);
		$time_total = $time_end - $time_start;
		$rpt=['d1'=>$documentofersl,'d2'=>$http_statuscod,'d3'=>$http_status,'D8'=>$info,'d9'=>$filename,'d6'=>$rsl,'d7'=>$time];
		$rsl=true;
		*/
		//return false;
		//return true;
...
// +---------------------------------------------------------------------------+
		...
		$rsmymclitc=$this->facturacionsendbajaws_db2_model->datos_emisor($fecha);
	...
	function genera_filejson_rc($data){
		...
		$datos=$this->datos_emisor($fecha);
		$tde=$datos['tde'];
		$rsnrorca=$this->facturacionsendbajaws_db2_model->cuentadocurc($fecha);
	...
	function genera_filejson_ra($data){
		...
		$datos=$this->datos_emisor($fecha);
		$tde=$datos['tde'];
		$rsnrorca=$this->facturacionsendbajaws_db2_model->cuentadocura($fecha);
...
/*
	...
	function optener_status_ticket($ticket){
...
function get_vereventosbajafe(){
	...
	if ($valid == TRUE) { 
		...
		$tipdoc=$this->input->post('tipdoc', true);
		$rsdocxgj=$this->facturacionsendbajaws_db2_model->get_vereventosbajafe($nropdc,$nrosere,$nrocor,$tipdoc);	
		if(($tipdoc=='08')||($tipdoc=='07')){
			$rsdocref=$this->facturacionsendbajaws_db2_model->get_verdocumentosrefncnd($nropdc,$nrosere,$nrocor,$tipdoc);
