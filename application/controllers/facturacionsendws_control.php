		...
		if ($valid == TRUE) { 
			...
			if($rsdocxgj!=false){
				while (odbc_fetch_row($rsdocxgj)) {
					...
					$body[]=array("SCTFECEM"=>$SCTFECEM,"SCTESUCA"=>$SCTESUCA,"SCTETDOC"=>$SCTETDOC,"SCTEPDCA"=>$SCTEPDCA,"SCTCRZSO"=>$SCTCRZSO,"SCTESERI"=>$SCTESERI,"SCTECORR"=>$SCTECORR,"SCTCTMON"=>$SCTCTMON,"SCTGNETO"=>number_format($SCTGTOTA,2,".",","),"SCTCSTST"=>$SCTCSTST,"SCTECIAA"=>$SCTECIAA,"SCTEFEC"=>$SCTEFEC,"nroevento"=>$nroevento);
				}	
				$data['msg']='Con Datos';
				$data['datos']=$body;
				$data['existe'] = 0;
...
// +---------------------------------------------------------------------------+				
			...
			$SCTGTOTAA='';
			$infoAnticipo=array();
			$dat['SCTECIAA']=$SCTECIAA;$dat['SCTESUCA']=$SCTESUCA;$dat['SCTEPDCA']=$SCTEPDCA;$dat['SCTETDOC']=$SCTETDOC;
			$stfeanti=$this->facturacionsendws_db2_model->get_anticiporelacionado_fe($dat);
			$ASCTESERI = '';$ASCTECORR = '';$ASCTETDOC = '';$ASCTGTOTA = '';$ASCTCTMON = '';$ASCTFECEM = '';$ASCTECRUC = '';$ASCTETPDO = '';
			if($stfeanti>0){
				$stfedataanti=$this->facturacionsendws_db2_model->get_anticipos_fe($dat);
				$ident=0;
...
//FN=Factura Normal,FS=Factura Servicio,FG=Factura Gratuita,FE=Factura Exportacion,FA=Factura Anticipo
	...
	function plantillaJSONFE(){		
		...
		$datos['cia']=$this->security->xss_clean($this->input->post('cia', true));
		...
		if($data!=""){
			...
			if($filesjs!=''){
				...
				$usuario=$datos_ws['usuario'];
				$clave=$datos_ws['clave'];
				$url=$datos_ws['url'];
	...
	function data_fact01($data,$SCTIPFAC){
		...
		$dat['SCTECIAA']=$data['SCTECIAA'];$dat['SCTESUCA']=$data['SCTESUCA'];$dat['SCTEPDCA']=$data['valorAdicional5'];$dat['SCTETDOC']=$data['codTipoDocumento'];
		$stfeanti=$this->facturacionsendws_db2_model->get_anticiporelacionado_fe($dat);
...
function send_mail_fe_demo(){
	...
	$hh= gmdate("H-i-s", time() - 18000);
		...
		function optener_status_webservice(){
			...
			//$result=$this->send_mail_fe();
			$result=$this->send_mail_fe_demo();
...
//se debe revisar este caso con james para levantar las observaciones 20191031 1824
	foreach ($datadet as $cad ) {
		$detallepcdfe[]=array("numeroItem"=>$cad['numeroItem'],
			...
			"cantidadItems"=>$cad['cantidadItems'],
			"unidad"=>$cad['unidad'],
			"valorUnitario"=>$cad['valorUnitario'],//0.00
			"precioVentaUnitario"=>$cad['precioVentaUnitario'],//0.00
			"totalImpuestos"=>array(
				array(
					"idImpuesto"=>$cad['idImpuesto'],//9996
					"montoImpuesto"=>'0.00',// $cad['montoImpuesto']
					"tipoAfectacion"=>'31',//$cad['tipoAfectacion'] en as400 esta con 13 se debe corregir
					"montoBase"=>'125.00',//montoBase $cad['valorVenta']
					"porcentaje"=>'0.00')),//$cad['porcentaje']
			"valorVenta"=>'125.00',//$cad['valorVenta'], si es un producto el valor total de este
			"valorRefOpOnerosas"=>'125.00',//$cad['valorVenta'],
			"montoTotalImpuestos"=>'0.00');//$cad['montoTotalImpuestos']
	...
	$documentofe01 = ['factura'=>array(
		...
		"CAB"=>array("gratuitas"=>array("codigo"=>$dato['codigoc'],
			"totalVentas"=>'125.00',//$dato['totalVentasc'] Aqui esta apareciendo total venta gratuita 0.00 en as400 
		),
		"totalImpuestos"=>array(array("idImpuesto"=>$dato['idImpuestoc'],
			"montoImpuesto"=>$dato['montoImpuestoc']
		)),
...
function optener_cdr_fe(){
	...
	if($documentofersl===false){
		...
		$body[]=array("dato1"=>$codigo .' '.$mensaje,"dato2"=>$xmlcdr);
		$data['msg']='Con Datos';
		$data['datos']=$body;
		$data['existe'] = 0;
	}
