...
// +---------------------------------------------------------------------------+        
	...
	function get_anulacionmanual($limit,$fecha){
		...
		//$fecha=(int)$fecha;
		if($limit>0){ 
			$sql ="select b.SCTEFEC,b.SCTESUCA,b.SCTEPDCA,b.SCTETDOC,b.SCTESERI,b.SCTECORR,b.SCTEALMA,b.SCTCSTST,b.SCTTDREF FROM LIBPRDDAT.MMYHREL0 a inner join LIBPRDDAT.SNT_CTRAM b on CASE WHEN a.YHTIPDOC='07' THEN (select IANROPDC FROM LIBPRDDAT.MMIAREP WHERE IACODCIA='10' AND IACODSUC=a.YHSUCDOC AND IANROREF=a.YHNROPDC) ELSE a.YHNROPDC END=b.SCTEPDCA and CASE WHEN a.YHTIPDOC='07' THEN 'F'||SUBSTRING(a.YHNUMSER,2) WHEN a.YHTIPDOC='08' THEN 'F'||SUBSTRING(a.YHNUMSER,2) ELSE a.YHNUMSER END=b.SCTESERI and a.YHNUMCOR=b.SCTECORR and a.YHTIPDOC=b.SCTETDOC WHERE a.YHSTS='I' AND a.YHCODCIA='10' and a.YHFECDOC ".$fecha." and b.SCTCSTS='A' limit ".$limit;
...
$query="a.YHFECDOC ".$fecha." and b.SCTEPDCA=".$data['nropdc']." and b.SCTETDOC='".$data['tipdoc']."' and b.SCTESERI='".$data['serie']."' and b.SCTECORR='".$data['correl']."' and b.SCTEALMA='".$data['codalm']."' and b.SCTCSTST='".$data['stsdoct']."'";
			...
			$sql ="select b.SCTEFEC,b.SCTESUCA,b.SCTEPDCA,b.SCTETDOC,b.SCTESERI,b.SCTECORR,b.SCTEALMA,b.SCTCSTST,b.SCTTDREF FROM LIBPRDDAT.MMYHREL0 a inner join LIBPRDDAT.SNT_CTRAM b on CASE WHEN a.YHTIPDOC='07' THEN (select IANROPDC FROM LIBPRDDAT.MMIAREP WHERE IACODCIA='10' AND IACODSUC=a.YHSUCDOC AND IANROREF=a.YHNROPDC) ELSE a.YHNROPDC END=b.SCTEPDCA and CASE WHEN a.YHTIPDOC='07' THEN 'F'||SUBSTRING(a.YHNUMSER,2) WHEN a.YHTIPDOC='08' THEN 'F'||SUBSTRING(a.YHNUMSER,2) ELSE a.YHNUMSER END=b.SCTESERI and a.YHNUMCOR=b.SCTECORR and a.YHTIPDOC=b.SCTETDOC WHERE a.YHSTS='I' AND a.YHCODCIA='10' and b.SCTCSTS='A' AND  ".$query;
...
// +---------------------------------------------------------------------------+     
	function documentoxgenerarjson($datas){
		...
		if($fecha!=''){	
			...
			$sql ="select a.SCTEFEC,a.SCTCTMON,a.SCTESERI,a.SCTECORR,a.SCTETDOC,a.SCTETPDO,a.SCTCNRUC,a.SCTGNETO,a.SCTCCIMP,a.SCTMTIMP,a.SCTGTOTA,a.SCTTDREF,a.SCTCSTST,a.SCTEALMA,a.SCTEPDCA,a.SCTFECEM FROM LIBPRDDAT.SNT_CTRAM a where a.SCTCSTS IN ('I','A') and a.SCTFECEM='".$fecha."' ".$query;
	...
	function datos_emisor($fecha){

		include("application/config/conexdb_db2.php");

		if($fecha!=''){				
			$sql ="select distinct(c.SCTERZSO) as SCTERZSO,c.SCTECRUC,c.SCTEUBIG,c.SCTEDIRE,c.SCTENCOM,c.SCTFECEM from LIBPRDDAT.SNT_CTRAM c where c.SCTFECEM='".$fecha."'";
			$dato = odbc_exec($dbconect, $sql)or die("<p>" . odbc_errormsg());
			if (!$dato) {
				$data = FALSE;
			} else {            
	...
	function cuentadocurc($fecha){
		...
		//$sql ="select count(SCTCSTS) as nrobolanu FROM LIBPRDDAT.SNT_CTRAM a where a.SCTFECEM='".$fecha."' and a.SCTECIAA='10' and a.SCTETDOC='03' and a.SCTCSTS='I' ";			
			$sql ="select count(SCTCSTS) as nrobolanu FROM LIBPRDDAT.SNT_CTRAM a where a.SCTECIAA='10' and a.SCTETDOC in ('03','07','08') and SCTTDREF in('0','3') and a.SCTCSTST IN ('I','N') and SCTCFECB=$fechae";
			$dato = odbc_exec($dbconect, $sql)or die("<p>" . odbc_errormsg());
			if (!$dato) {
				$data = FALSE;
			} else {            
	...
	function cuentadocura($fecha){
		...
		//$sql ="select count(SCTCSTS) as nroanu FROM LIBPRDDAT.SNT_CTRAM a where a.SCTFECEM='".$fecha."' and a.SCTECIAA='10' and a.SCTETDOC<>'03' and a.SCTCSTS='I'";			
			$sql ="select count(SCTCSTS) as nroanu FROM LIBPRDDAT.SNT_CTRAM a where a.SCTECIAA='10' and a.SCTETDOC<>'03' and a.SCTCSTST IN ('I','N') and SCTTDREF in('0','1') and SCTCFECB=$fechae";
			$dato = odbc_exec($dbconect, $sql)or die("<p>" . odbc_errormsg());
			if (!$dato) {
				$data = FALSE;
			} else {            
...
// +---------------------------------------------------------------------------+        
	...
	function get_vereventosbajafe($nropdc,$nrosere,$nrocor,$tipdoc){
		include("application/config/conexdb_db2.php");
		$codcia=$this->session->userdata('codcia');                                                       
